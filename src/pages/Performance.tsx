...
import { format, subMonths, differenceInDays, parseISO, startOfMonth, endOfMonth, isWithinInterval } from 'date-fns';
...
export default function PerformancePage() {
  ...
  // Monthly heatmap: category × month matrix
  const monthlyHeatmap = useMemo(() => {
    if (snapshots.length < 2) return { months: [] as string[], categories: [] as string[], matrix: {} as Record<string, Record<string, number>> };

    // Get unique months from snapshots
    const monthMap = new Map<string, { date: string; nw: number }>();
    for (const s of snapshots) {
      const monthKey = format(parseISO(s.snapshot_date), 'yyyy-MM');
      monthMap.set(monthKey, { date: s.snapshot_date, nw: Number(s.net_worth) });
    }
    const months = [...monthMap.keys()].sort().slice(-12);

    // For category breakdown, use breakdown_json if available, otherwise use current allocation
    const categories = [...new Set(assetPerformance.map(a => a.category))];

    // Build matrix of month-over-month returns per category
    // Since we don't have per-category monthly data in snapshots, approximate using snapshot net_worth changes
    const matrix: Record<string, Record<string, number>> = {};
    for (const cat of categories) {
      matrix[cat] = {};
      for (let i = 1; i < months.length; i++) {
        const prev = monthMap.get(months[i - 1]);
        const curr = monthMap.get(months[i]);
        if (prev && curr && prev.nw > 0) {
          // Approximate: distribute portfolio return to categories proportionally
          const catAssets = assetPerformance.filter(a => a.category === cat);
          const catWeight = catAssets.reduce((s, a) => s + a.current, 0) / (overview?.total_current_value || 1);
          const portfolioReturn = ((curr.nw - prev.nw) / prev.nw) * 100;
          matrix[cat][months[i]] = portfolioReturn * catWeight * categories.length; // Scale up
        }
      }
    }

    return { months: months.slice(1), categories, matrix };
  }, [snapshots, assetPerformance, overview]);
  ...
  const rollingReturns = useMemo(() => {
    ...
    return periods.map(p => {
      ...
      return { period: p.label, returnPct: ((currentVal - pastVal) / pastVal) * 100 };
    });
  }, [snapshots]);
  ...
  const { maxDrawdown, recoveryDays } = useMemo(() => {
    ...
    let peak = 0, maxDd = 0, troughDate = '';
    for (const s of snapshots) {
      const nw = Number(s.net_worth);
      if (nw > peak) peak = nw;
      const dd = peak > 0 ? ((peak - nw) / peak) * 100 : 0;
      if (dd > maxDd) { maxDd = dd; troughDate = s.snapshot_date; }
    }
    ...
    if (troughDate) {
      ...
      if (recovered) recDays = differenceInDays(parseISO(recovered.snapshot_date), parseISO(troughDate));
    }
  ...
  const contributionSplit = useMemo(() => {
    ...
    return { contributions: investedChange, marketReturn: totalChange - investedChange, total: totalChange };
  }, [snapshots]);

  const getHeatColor = (val: number) => {
    if (val > 5) return 'bg-positive/80 text-positive-foreground';
    if (val > 2) return 'bg-positive/40 text-foreground';
    if (val > 0) return 'bg-positive/20 text-foreground';
    if (val > -2) return 'bg-destructive/20 text-foreground';
    if (val > -5) return 'bg-destructive/40 text-foreground';
    ...
    <AppLayout>
      <div className="p-4 md:p-6 lg:p-8 space-y-6">
        ...
        <LearnMoreDialog
          ...
          sections={[
            { heading: 'CAGR (Compound Annual Growth Rate)', content: 'Measures the annualized return of an investment over time, assuming profits are reinvested. Formula: (End Value / Begin Value)^(1/Years) - 1. A CAGR of 12% means your investment grew at 12% annually on average.' },
            { heading: 'XIRR (Extended Internal Rate of Return)', content: 'Calculates the annualized return accounting for irregular cash flows (investments and withdrawals at different dates). More accurate than CAGR for portfolios with multiple transactions.' },
            { heading: 'Max Drawdown', content: 'The largest peak-to-trough decline in portfolio value during a period. A 10% drawdown means the portfolio fell 10% from its highest point before recovering. Lower drawdowns indicate more stable returns.' },
            { heading: 'Rolling Returns', content: 'Shows your portfolio return over trailing periods (1M, 3M, 6M, 1Y). Unlike point-to-point returns, rolling returns give a more complete picture of performance consistency.' },
            { heading: 'Contribution vs Market Return', content: 'Splits your portfolio growth into two parts: new money you invested (contributions) and actual investment gains/losses (market return). Helps understand how much growth came from saving vs. investing.' },
          ]}
        />
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base flex items-center gap-2">
                Max Drawdown
                <Tooltip><TooltipTrigger><HelpCircle className="h-3.5 w-3.5 text-muted-foreground" /></TooltipTrigger>
                  <TooltipContent className="max-w-[250px]">Maximum peak-to-trough decline in portfolio value.</TooltipContent>
                </Tooltip>
              </CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card>
            <CardHeader className="pb-2">
              <CardTitle className="text-base flex items-center gap-2">
                Contribution vs Market Return
                <Tooltip><TooltipTrigger><HelpCircle className="h-3.5 w-3.5 text-muted-foreground" /></TooltipTrigger>
                  <TooltipContent className="max-w-[250px]">New money invested vs. investment gains/losses.</TooltipContent>
                </Tooltip>
              </CardTitle>
            </CardHeader>
            <CardContent>
                ...
                <div className="space-y-2">
                  ...
                  {contributionSplit.total !== 0 && (
                    <div className="flex h-3 rounded-full overflow-hidden bg-muted mt-2">
                      <div className="bg-primary" style={{ width: `${Math.max(0, Math.min(100, (contributionSplit.contributions / Math.abs(contributionSplit.total)) * 100))}%` }} />
                      <div className={cn(contributionSplit.marketReturn >= 0 ? "bg-positive" : "bg-destructive")}
        ...
        <Tabs defaultValue="gainers" className="space-y-4">
          <TabsList>
            ...
            <TabsTrigger value="heatmap" className="gap-1.5"><BarChart3 className="h-3.5 w-3.5" /> Category</TabsTrigger>
            <TabsTrigger value="monthly" className="gap-1.5"><Grid3X3 className="h-3.5 w-3.5" /> Monthly</TabsTrigger>
          </TabsList>
          ...
          <TabsContent value="heatmap">
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-base">Category Performance</CardTitle></CardHeader>
              <CardContent>
            ...
            <Card>
              <CardHeader className="pb-2">
                <CardTitle className="text-base flex items-center gap-2">
                  ...
                  <Tooltip><TooltipTrigger><HelpCircle className="h-3.5 w-3.5 text-muted-foreground" /></TooltipTrigger>
                    <TooltipContent className="max-w-[250px]">Estimated monthly returns by category based on portfolio snapshots.</TooltipContent>
                  </Tooltip>
                </CardTitle>
              </CardHeader>
              <CardContent>
                {monthlyHeatmap.months.length > 0 ? (
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs">
                      <thead>
                        <tr>
                          <th className="text-left p-2 text-muted-foreground font-medium">Category</th>
                          {monthlyHeatmap.months.map(m => (
                            <th key={m} className="p-2 text-muted-foreground font-medium text-center">
                      ...
                      <tbody>
                        {monthlyHeatmap.categories.map(cat => (
                          <tr key={cat}>
                            <td className="p-2 font-medium text-foreground">{cat}</td>
                            {monthlyHeatmap.months.map(m => {
                              const val = monthlyHeatmap.matrix[cat]?.[m];
                              return (
                  ...
                  <p className="text-center text-sm text-muted-foreground py-8">Need at least 2 months of snapshot data</p>
                )}
              </CardContent>
            </Card>
          </TabsContent>
        ...
        {snapshots.length > 1 && (
          <Card>
            <CardHeader className="pb-2"><CardTitle className="text-base">Net Worth Trend</CardTitle></CardHeader>
            <CardContent>
              <ResponsiveContainer width="100%" height={300}>
